...
def openf1_intervals_supabase():
    ...
    def sync_intervals() -> None:
        """Sync interval data from OpenF1 API to Supabase."""
        ...
        if not rows:
            print("No interval data to upsert")
            return

        print(f"Upserting {len(rows)} interval records for session {session_key}")
        result = (
            sb.table("intervals")
            .upsert(rows, on_conflict="session_key,driver_number,date")
            .execute()
        )
