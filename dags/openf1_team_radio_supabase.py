...
def openf1_team_radio_supabase():
    ...
    def sync_team_radio() -> None:
        """Sync team radio data from OpenF1 API to Supabase using PostgreSQL connection."""
        ...
        # Fetch team radio data for target drivers
        ...
        if not rows:
            logger.info("No team radio data to upsert")
            return

        logger.info(
            f"Upserting {len(rows)} team radio records for session {session_key}"
        )
        ...
        try:
            ...
            with psycopg2.connect(**conn_kwargs) as conn:
                with conn.cursor() as cursor:
                    ...
                    conn.commit()
                    logger.info(f"Successfully upserted {len(rows)} team radio records")

        except Exception as e:
            logger.error(f"Error upserting team radio data: {str(e)}")
            raise
